...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, groups=1,
        ...
        self.relu = nn.ReLU(inplace=True)
        self.stride = stride
        self.index_mask_1 = torch.ones([1, width, 1, 1])
        self.index_mask_2 = torch.ones([1, width, 1, 1])

    def forward(self, x):
        out = self.conv1(x)
        out = self.bn1(out)
        out = self.relu(out)
        mask = self.index_mask_1.expand(out.size()).cuda()
        out = out.mul(mask)

        out = self.conv2(out)
        out = self.bn2(out)
        out = self.relu(out)
        mask = self.index_mask_2.expand(out.size()).cuda()
        out = out.mul(mask)
...
class ResidualStage(nn.Module):
    ...
    def res_block_forward(self, x, block, check_downsample=False):
        ...
        if check_downsample and self.downsample is not None:
            identity = self.downsample(x)
            mask = self.index_mask.expand(identity.size()).cuda()
            identity = identity.mul(mask)
        out = block(x)
        out += identity
        out = self.relu(out)
...
class ResNet(nn.Module):
    ...
    def _initialize_old_weights(self, model_weight):
        ...
        new_keys = []
        for item in my_keys:
            if 'num_batches_tracked' not in item:
                new_keys.append(item)
        for i, (k, v) in enumerate(model_weight.items()):
            if 'running' in k:
                my_weight[new_keys[i + 2]] = v
            elif 'bn' in k:
                my_weight[new_keys[i - 2]] = v
