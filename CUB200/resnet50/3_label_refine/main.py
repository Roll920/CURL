...
def main():
    ...
    D2 = DeepDecipher(len(train_loader), FLAGS['class_num'], 1, int(len(dsets['train']) / 6))
...
def train(train_loader, model, optimizer, epoch, D2):
    ...
    for i, (input, label, index) in enumerate(train_loader):
        ...
        T = 1.0
        loss1 = torch.mean(
            T * T * torch.sum(softmax(output / T) * (logsoftmax(output / T) - logsoftmax(d2_label / T)), dim=1))
